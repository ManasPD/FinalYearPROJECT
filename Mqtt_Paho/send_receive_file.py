...
topic = "/data/files"
qos = 1
data_block_size = 25000
fo = open(filename, "rb")
#file_out = "copy-11" + filename
fout = open("D://Recreational/123.jpg", "wb")  # use a different filename
...
def c_publish(client, topic, out_message, qos):
    ...
    if res == 0:  # published ok
        if wait_for(client, "PUBACK", running_loop=True):  #wait for on_publish
